...
int main()
{
	...
	FILE *f2;
	f2=fopen("Parameters-Empirical.txt","w");
	FILE *f3I;
	f3I=fopen("Infection-daily-Empirical.txt","w");
	FILE *f3V;
	f3V=fopen("Vaccination-daily-Empirical.txt","w");
	FILE *f4I;
	f4I=fopen("TotalInfection-Season-Empirical.txt","w");
	FILE *f4V;
	f4V=fopen("TotalVaccination-Season-Empirical.txt","w");
	FILE *f6I;
	f6I=fopen("TotalSSInfection-Season-Empirical.txt","w");
	FILE *f6V;
	f6V=fopen("TotalSSVaccination-Season-Empirical.txt","w");
	FILE *f8V;
	f8V=fopen("SS-SelfVac-Empirical.txt","w");
	FILE *f9;
	f9=fopen("Degree-Infection-Empirical.txt","w");
	FILE *f10;
	f10=fopen("Degree-Vaccination-Empirical.txt","w");
	FILE *f11;
	f11=fopen("NumSS-NumSSCont-Empirical.txt","w");
	FILE *f12;
	f12=fopen("NumberDegree-Empirical.txt","w");
	FILE *f13;
	f13=fopen("SS-SelectedRandom-Empirical.txt","w");
	FILE *f14;
	f14=fopen("SS-NNSelected-Empirical.txt","w");
	FILE *f15;
	f15=fopen("Degree-avgpeps-Empirical.txt","w");
	FILE *f16;
	f16=fopen("SSChar-Empirical.txt","w");
	FILE *f17;
	f17=fopen("Incentives-Empirical.txt","w");
	FILE *f18;
	f18=fopen("RecruitV-Empirical.txt","w");
	srand(time(NULL));
	...
	int Nyears=500; // number of years to run for
	fprintf(f2,"NYears %d \n",Nyears);
	int NSims=10; //number of simulations to run
	fprintf(f2,"NSims %d \n",NSims);
	float q=0.69; // for Ro and the calc of avgp
	fprintf(f2,"q %f \n",q);
	...
	for(Sim=0;Sim<NSims;Sim++)
	{
		...
		if(Sim==0)
		{
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-1-RW.txt","r");			
			f1=fopen("Contacts-SD-1-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
				itemp=(int)ftemp;
				ind[jj].contact=(int*) malloc(itemp*sizeof(int));
				ind[jj].Q=itemp;
				ind[jj].Qcn=0;
				while ((!feof (f1))&& (itemp>0))
				{  
					fscanf (f1, "%f", &ftemp);	
					itemp2=(int)ftemp;
					ind[jj].contact[ind[jj].Qcn]=itemp2;
					ind[jj].Qcn++;
					itemp--;
				}
				jj++;
		...
		else if (Sim==NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-2-RW.txt","r");			
			f1=fopen("Contacts-SD-2-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==2*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-3-RW.txt","r");			
			f1=fopen("Contacts-SD-3-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==3*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-4-RW.txt","r");			
			f1=fopen("Contacts-SD-4-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==4*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-5-RW.txt","r");			
			f1=fopen("Contacts-SD-5-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==5*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-6-RW.txt","r");			
			f1=fopen("Contacts-SD-6-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==6*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-7-RW.txt","r");			
			f1=fopen("Contacts-SD-7-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==7*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-8-RW.txt","r");			
			f1=fopen("Contacts-SD-8-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==8*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-9-RW.txt","r");			
			f1=fopen("Contacts-SD-9-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		else if (Sim==9*NSims/10)
		{
			...
			FILE *f0;
			FILE *f1;
			f0=fopen("ContactDegree-SD-10-RW.txt","r");			
			f1=fopen("Contacts-SD-10-RW.txt","r");
			jj=0;
			while (!feof (f0))
			{   	
				fscanf (f0, "%f", &ftemp);
		...
		for(year=0;year<(Nyears+TransYear);year++)
		{
			...
			if(year>0)
			{
				...
				for(ii=0;ii<N;ii++)
				{
					...
					ppv=(atan(3950*PVPN/QALY)+(PI/2))/PI;
					if(urand()<ppv)
					{
						ftemp=urand();
			...
			{ 
				if((day-75==0)||(day-89==0)||(day-97==0)||(day-103==0)||(day-110==0)||(day-117==0)||(day-118==0)||(day-119==0)||(day-120==0)||(day-121==0)||(day-122==0))//randomly infecting people starting first part of november
				{
					itemp=rand()%N;
				...
				for(ii=0;ii<N;ii++) //set seasonal transmission for each individual and vaccination
				{
					...
					ind[ii].TI++;
					ind[ii].TC++;
					ind[ii].p=ind[ii].avgp*(1+ind[ii].sp*sin(2*PI*(day-120)/365));
					ind[ii].TItemp=ind[ii].TI;
					ind[ii].TCtemp=ind[ii].TC;
					ind[ii].pepstemp=ind[ii].peps;
				}
