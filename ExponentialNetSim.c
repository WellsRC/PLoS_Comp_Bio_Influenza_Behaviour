...
int main()
{
	...
	FILE *f2;
	f2=fopen("Parameters-Exponential.txt","w");
	FILE *f3I;
	f3I=fopen("Infection-daily-Exponential.txt","w");
	FILE *f3V;
	f3V=fopen("Vaccination-daily-Exponential.txt","w");
	FILE *f4I;
	f4I=fopen("TotalInfection-Season-Exponential.txt","w");
	FILE *f4V;
	f4V=fopen("TotalVaccination-Season-Exponential.txt","w");
	FILE *f6I;
	f6I=fopen("TotalSSInfection-Season-Exponential.txt","w");
	FILE *f6V;
	f6V=fopen("TotalSSVaccination-Season-Exponential.txt","w");
	FILE *f8V;
	f8V=fopen("SS-SelfVac-Exponential.txt","w");
	FILE *f9;
	f9=fopen("Degree-Infection-Exponential.txt","w");
	FILE *f10;
	f10=fopen("Degree-Vaccination-Exponential.txt","w");
	FILE *f11;
	f11=fopen("NumSS-NumSSCont-Exponential.txt","w");
	FILE *f12;
	f12=fopen("NumberDegree-Exponential.txt","w");
	FILE *f13;
	f13=fopen("SS-SelectedRandom-Exponential.txt","w");
	FILE *f14;
	f14=fopen("SS-NNSelected-Exponential.txt","w");
	FILE *f15;
	f15=fopen("Degree-avgpeps-Exponential.txt","w");
	FILE *f16;
	f16=fopen("SSChar-Exponential.txt","w");
	FILE *f17;
	f17=fopen("Incentives-Exponential.txt","w");
	srand(time(NULL));	
	...
	fprintf(f2,"NSims %d \n",NSims);
	float q=0.575; // for Ro and the calc of avgp
	fprintf(f2,"q %f \n",q);
	...
	float sp=0.0525; //seasonal change in the transmission rate corresponding to the uniform random network	
	fprintf(f2,"sp %f \n",sp);
		...
		for(year=0;year<(Nyears+TransYear);year++)
		{
			...
			if(year>0)
			{
				...
				for(ii=0;ii<N;ii++)
				{
					...
					ppv=(atan(4000*PVPN/QALY)+(PI/2))/PI;
					if(urand()<ppv)
					{
						ftemp=urand();
			...
			{ 
				if((day-68==0)||(day-89==0)||(day-96==0)||(day-99==0)||(day-117==0)||(day-119==0)||(day-121==0))//randomly infecting people starting first part of november
				{
					...
					if(day<100)
					{
						while((ind[itemp].state!=0)&&(ind[itemp].Q>2))
						{
							itemp=rand()%N;
						}
					}
				...
				for(ii=0;ii<N;ii++) //set seasonal transmission for each individual and vaccination
				{
					...
					ind[ii].TI++;
					ind[ii].TC++;
					ind[ii].p=ind[ii].avgp*(1+ind[ii].sp*sin(2*PI*(day-122)/365));
					ind[ii].TItemp=ind[ii].TI;
					ind[ii].TCtemp=ind[ii].TC;
					ind[ii].pepstemp=ind[ii].peps;
				}
